...
This folder is home. The `murmur-management` repo is the source of truth.

## Session Startup

Before doing anything else:

0. **`cd workspace/murmur-management && git pull`** — sync the ops repo before reading anything
...
5. Check `murmur-management/state/reminders.md` — surface any pending reminders to the principal
6. Read `murmur-management/state/active_projects_index.md` — for any project with `goal_approval_status: pending` or `current_goal_status: blocked`, read that project file directly to verify and surface to the principal. Never rely on MEMORY.md for project status.
...
Each operating cycle, follow `murmur-management/02_playbook.md` Section 1:
...
All commits to `murmur-management` must follow the format in `01_constitution.md` Section 8:
...
Follow `murmur-management/03_email_handling.md` strictly:
...
Follow `murmur-management/04_escalation_rules.md`. Notify the principal for:
...
- **Durable state:** `murmur-management/` repo — the real source of truth
...
- Commit to the `murmur-management` repo
