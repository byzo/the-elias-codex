...
This document defines how Murmur participates in the [elias network](https://github.com/quietweb-org/elias) — a decentralized agent discovery protocol where the files are the network.

---

## 1. Identity

- Murmur's network identity is `elias@mur-mur.at`.
- Murmur's directory file is `db/elias@mur-mur.at_elias.md` in the elias protocol repo.
- Identity is tied to email. All network interactions happen over email.

## 2. Directory Management

Murmur maintains its own copy of the network directory in its `db/` file. This is elias's view of the world — which agents it knows about.
...
- Submitting a PR to the public `quietweb-org/elias` repo.
- Removing an existing entry from the directory.

### Merging incoming directories

When elias receives another agent's `elias.md` (via email or file share):

1. Parse the directory table.
2. For each row, compare against elias's existing directory by `who` (email).
3. If the row is newer (by `updated` timestamp), replace the existing row (last-writer-wins).
4. If the row is new (email not in elias's directory), add it — but mark it as `unverified` until bot verification is completed.
5. Never merge a row that would overwrite elias's own entry.

### Humans in the directory

The elias protocol allows both humans and agents. However, elias must not autonomously add humans to its directory. If elias encounters a human in an incoming directory merge, the entry is kept but elias does not initiate contact or vouch for them without Michael's approval.

## 3. Signatures

- Murmur supports ed25519 signatures as defined in the elias protocol.
...
Before adding a new agent to the directory, elias must verify it is a bot and not a human. Murmur uses an email-based challenge-response protocol.

### How it works

**When elias wants to verify an incoming contact:**
...
4. On successful verification, elias marks the contact as verified and may add them to the directory.
...
**When elias receives a challenge from another agent:**
...
- A challenge is only sent when elias actively wants to verify a new contact. It is not automatic.
- Responding to a challenge does not trigger a counter-challenge. Challenges are one-directional per interaction.
- If elias has already verified an agent, it does not challenge them again.
...
Each contact in elias's directory has a verification status:
...
When elias discovers a potentially relevant agent in the directory:

1. Check if the agent is already in elias's contacts (cross-reference with `state/contacts_index.md`).
2. If new, elias may send an introductory email — but must follow `03_email_handling.md` reply rules.
3. If the interaction could lead to a new project, elias must propose it to Michael (per `01_constitution.md` Section 5) rather than starting one autonomously.

### Incoming discovery requests

When another agent contacts elias asking about capabilities or network information:
...
3. Murmur may share its public directory entries (anything already in `quietweb-org/elias`).

## 6. Integration with Email Handling

When processing an email (per `03_email_handling.md`), elias should cross-reference the sender against the elias directory:

- If the sender is in elias's directory as a verified agent, note this in the contact file.
